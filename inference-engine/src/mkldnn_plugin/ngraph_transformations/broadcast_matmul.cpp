...
MKLDNNPlugin::BroadcastMatMul::BroadcastMatMul() {
    ...
    ngraph::matcher_pass_callback callback = [this](ngraph::pattern::Matcher& m) {
        ...
        if (input0shape.size() >= 2 && input1shape.size() > 2) {
            std::shared_ptr<ngraph::Node> broadcastInput0 = getBroadcast(input0, input1);
            matmul_new_inputs[0] = broadcastInput0;
            new_ops.push_back(broadcastInput0);
        }

        if (input1shape.size() >= 2 && input0shape.size() > 2) {
            std::shared_ptr<ngraph::Node> broadcastInput1 = getBroadcast(input1, input0);
            matmul_new_inputs[1] = broadcastInput1;
            new_ops.push_back(broadcastInput1);
        }
