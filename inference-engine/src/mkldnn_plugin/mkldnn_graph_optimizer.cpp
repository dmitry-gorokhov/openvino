...
void MKLDNNGraphOptimizer::FuseConvolutionMatMulAndBias(MKLDNNGraph &graph) {
    ...
    auto isSuitableChildNode = [&](const MKLDNNNodePtr& parentNode, const MKLDNNNodePtr& childNode) {
        ...
        if (biasNode->getType() != Input || !biasNode->isConstant())
            return false;
...
//        }
    ...
    while (parent != graphNodes.end()) {
        ...
        if (parentNode->getName() == "Convolution_7772")
...
void MKLDNNGraphOptimizer::FusePerformedAsScaleShiftAndFakeQuantize(MKLDNNGraph &graph) {
    ...
    auto fuseScaleShiftAndFakeQuantizeNodes = [getConstPort](MKLDNNNodePtr parent, MKLDNNNodePtr child) {
        ...
        std::tie(scalesBuffer, shiftsBuffer) = parentEltwise->getScalesAndShifts(parent->getParentEdgesAtPort(1 - getConstPort(parent))[0]->getParent());

        scalesBuffer = makeAlignedBuffer(scalesBuffer.size(), scalesBuffer, 1);
        shiftsBuffer = makeAlignedBuffer(shiftsBuffer.size(), shiftsBuffer, 1);
