...
void MKLDNNNode::appendPostOps(mkldnn::post_ops& ops, int align) {
...
std::pair<std::vector<float>, std::vector<float>> MKLDNNNode::getScalesAndShifts(const MKLDNNNodePtr &parentNode) const {
    ...
    const auto constPort = getParentEdgesAtPort(0)[0]->getParent() == parentNode ? 1 : 0;
