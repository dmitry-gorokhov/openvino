...
void MKLDNNEltwiseNode::fuseInto(MKLDNNNodePtr& parentNode) {
    ...
    if (!specialConvolutionAddFusing && canBePerformedAsScaleShift(parentNode.get())) {
        std::tie(scales, shifts) = getScalesAndShifts(parentNode);
    }
    MKLDNNNode::fuseInto(parentNode);
}

void MKLDNNEltwiseNode::appendPostOps(mkldnn::post_ops& ops, int align) {
    ...
    if (getMKLDNNAlgorithm() != mkldnn::algorithm::undef) {
        ...
        const size_t chIdx = getOutputShapeAtPort(0).getRank() > 1 ? 1 : 0;
        scalesBuffer = makeAlignedBuffer(getOutputShapeAtPort(0).getDims()[chIdx], scales, align);
        if (getAlgorithm() != EltwisePrelu) {
            shiftsBuffer = makeAlignedBuffer(getOutputShapeAtPort(0).getDims()[chIdx], shifts, align);
        }
...
void MKLDNNEltwiseNode::appendBinPostOps(mkldnn::post_ops& ops, const std::vector<size_t>& binaryShape, std::vector<MKLDNNMemoryPtr>& binaryPostOpsMem) {
    ...
    auto appendBinary = [&](const mkldnn::algorithm alg, MKLDNNMemoryPtr &memPtr, const std::vector<float> &data) {
        ...
        DnnlBlockedMemoryDesc memoryDesc(Precision::FP32, policy == PerTensor ? Shape(broadcastBinaryShape) : Shape(binaryShape));
    ...
    case EltwisePowerStatic:
        if (beta != 1.0f) // Multiply if has scales
            appendBinary(mkldnn::algorithm::binary_mul, scalesMemory, scales);
        if (gamma != 1.0f) // Add only if has shifts
            appendBinary(mkldnn::algorithm::binary_add, shiftsMemory, shifts);
        break;
    case EltwisePrelu:
